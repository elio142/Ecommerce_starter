...
export default function ProductCard({ id, product, onRemoveFavorite }) {
  ...
  // Load wishlist status when user or product changes
  useEffect(() => {
  ...
  const handleWishlistToggle = () => {
    ...
    if (isWishlisted) {
      ...
      if (onRemoveFavorite) onRemoveFavorite(productData.id);
    } else {
      wishlistArray.push(productData.id);
    }

    localStorage.setItem(storageKey, JSON.stringify(wishlistArray));
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden group">
      <div className="relative h-48">
        ...
        <button
          ...
          className="cursor-pointer absolute top-2 right-2 p-2 rounded-full bg-white shadow-md hover:bg-gray-100"
        >
          <svg
